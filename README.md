...
Browser -> Railway TLS -> this Caddy service -> Authelia (Railway private network)
                                            ...
                                            -> private Tailscale Serve ports -> Hermes / ComfyUI
```

## What this repository provides

- Routes `AUTH_HOST` to the private Authelia service and `HERMES_HOST` and `COMFY_HOST` to separate private Tailscale Serve ports on the Windows node.
...
2. Tailscale MagicDNS and HTTPS enabled for the Windows node. Give the Railway Tailscale service an ephemeral auth key or OAuth client secret that can register `tag:railway-edge`; grant that tag access to the node's two Serve ports. Keep the key in Railway variables, never in Git. See [Tailscale grants](https://tailscale.com/docs/features/access-control/grants).
3. Hermes Agent and ComfyUI answering behind the node's private Tailscale Serve routes. The target ports on the Windows machine must have active listeners. Do not point public DNS at a Tailscale `100.x` address.
...
| `HERMES_TAILSCALE_HOST` | `desktop-friday.<tailnet>.ts.net` | Private Windows node hostname, without scheme or port |
| `HERMES_TAILSCALE_PORT` | `443` | HTTPS Serve port for Hermes; defaults to `443` |
| `COMFY_TAILSCALE_HOST` | `desktop-friday.<tailnet>.ts.net` | Same private Windows node hostname |
| `COMFY_TAILSCALE_PORT` | `8443` | HTTPS Serve port for ComfyUI; defaults to `443` if omitted |
...
| `TS_ACCEPT_DNS` | `true` | Accept tailnet DNS settings for the node hostname |
...
Merge a narrow grant into the existing tailnet policy after defining ownership of `tag:railway-edge` and checking for broader grants. In this deployment, the Windows node has `tag:desktop`:
...
[Tailscale Serve](https://tailscale.com/docs/features/tailscale-serve), [grants syntax](https://tailscale.com/docs/reference/syntax/grants)

## Windows private backhaul

The Windows machine serves Hermes on `https://desktop-friday.<tailnet>.ts.net:443` and ComfyUI on `https://desktop-friday.<tailnet>.ts.net:8443`. Confirm `tailscale serve status` shows both routes as **tailnet only**, not Funnel. Each route must proxy to a live local app port. Test both URLs from an allowed tailnet device with normal certificate verification. A disallowed tailnet device and an ordinary internet client must be unable to reach them directly. The separate Tailscale Service hostnames were not reachable through the userspace outbound proxy in this deployment, so Caddy targets the node's verified Serve ports.
...
docker run --rm --entrypoint caddy `
  ...
  -e HERMES_TAILSCALE_HOST=desktop-friday.example.ts.net `
  -e HERMES_TAILSCALE_PORT=443 `
  -e COMFY_TAILSCALE_HOST=desktop-friday.example.ts.net `
  -e COMFY_TAILSCALE_PORT=8443 `
